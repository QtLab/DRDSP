...
double Brusselator::operator()( const VectorXd& x, uint32_t k ) const {
	uint32_t i,j;
	if( !(k % 2) ) {
		j = k / (2*nX);
		i = k/2 - j * nX;
...
double Brusselator::Partials( const VectorXd& x, uint32_t k1, uint32_t k2 ) const {
...
double Brusselator::XDot( const VectorXd& x, uint32_t i, uint32_t j ) const {
	return A + X(x,i,j)*X(x,i,j)*Y(x,i,j) - (B+1.0)*X(x,i,j) + D1 * laplacianX(x,i,j);
}

double Brusselator::YDot( const VectorXd& x, uint32_t i, uint32_t j ) const {
	return B*X(x,i,j) - X(x,i,j)*X(x,i,j)*Y(x,i,j) + D2 * laplacianY(x,i,j);
}

double Brusselator::dXXDot( const VectorXd& x, uint32_t i, uint32_t j, uint32_t mu, uint32_t nu ) const {
...
double Brusselator::dXYDot( const VectorXd& x, uint32_t i, uint32_t j, uint32_t mu, uint32_t nu ) const {
