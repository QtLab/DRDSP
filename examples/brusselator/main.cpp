...
void ComputeReducedCGL( const Options& options );
void ComputeReducedFHN( const Options& options );
...
int main( int argc, char** argv ) {
	...
	//ComputeReducedCGL( options );
	ComputeReducedFHN( options );
...
void ComputeReducedCGL( const Options& options ) {
	...
	auto parameters = ParameterList( 1.0, 2.0, 3 );

	cout << "Generating data..." << endl;
	DataGenerator<GinzburgLandauFamily> dataGenerator;
	dataGenerator.initial.setRandom( ginzburgLandau.stateDim );
	dataGenerator.tStart = 0.0;
	dataGenerator.tInterval = 10.0;
	dataGenerator.print = 200;
	dataGenerator.dtMax = 0.0001;

	DataSystem data = dataGenerator.GenerateDataSystem( parameters, options.numThreads );

	auto data2 = MapMatrix( MatrixXd::Identity(2,data.dimension), data.dataSets[0] );
